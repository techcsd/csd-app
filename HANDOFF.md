...
## Ronda 28/07 — PROMPT-2-CSD-APP (2026-07-28) — Y3/Y4/Y5/Y6/Y7/Y9/Y10/Y11-app (FASES 1–8) — COMPLETA · build VERDE · SIN commit/APK
**Estado:** las 8 fases implementadas y `npm run build` VERDE (solo warnings NG8102 preexistentes). **NADA commiteado/pusheado, APK NO construida** (pedir a Xaviel). **PROMPT-1-SGC verificado APLICADO** en la BD compartida por introspección: `sgc.app_error_reports` (con `platform`), RPC `report_app_error`, módulo `tecnologia` + roles "Tecnología"/"Encargado de Tecnología", `es_tecnologia()` (codigo admin|tecnologia). Deps nuevas: `@capacitor/device@8` (device info + WebView version) y `write-excel-file@4` (Excel real, sin advisories — se descartó `xlsx` por su vuln de parser). `npm run apk` corre `npx cap sync android` → registra el plugin device nativo.
...
**FASE 3 (Y4 bomba en 0):** 3ª foto obligatoria en combustible (recibo+tablero+bomba). **Migración aplicada** `sql/2026-07-28-y4-combustible-foto-bomba.sql` (aditiva: columna `registros_combustible.foto_bomba_path` + param `p_foto_bomba_path DEFAULT NULL` en `registrar_combustible_app` vía DROP+CREATE, retrocompat — verificado 12 args). Files: `combustible.ts/.html`, `combustible.service.ts`, `combustible.model.ts`. **⚠️ Falta en SGC (web)**: mostrar la 3ª foto — el service ya hace `select('*')` así que `foto_bomba_path` YA llega; solo hay que (a) `combustible.model.ts` web: añadir `foto_bomba_path: string|null;`, (b) `pages/flota/combustible/combustible.ts` ~L412: añadir `this.combustibleService.getFotoUrl(row.foto_bomba_path)` como `bombaUrl` y pintarla junto a recibo/tablero en el HTML.
...
- [ ] Tras QA: `npm run apk` (bump + registra Y1) + `npm run apk:publish` + publicar/mínima en SGC.
