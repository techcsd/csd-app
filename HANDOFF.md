...
## Ronda 28/07 — PROMPT-4-CSD-APP (2026-07-28) — Y14/Y15-app: Proyectos + Cronograma (FASES 1–5) — COMPLETA · build VERDE · SIN commit/APK
**Estado:** 5 fases implementadas, `npm run build` VERDE (solo warnings NG8102 preexistentes). **NADA commiteado/pusheado, APK NO construida** (esperar OK de Xaviel — prompt nuevo). **PROMPT-3-SGC verificado APLICADO**: contrato de cronograma vivo (`docs/cronograma-diseno.md §15`) — RPCs `listar_cronograma`, `iniciar_tarea`, `completar_tarea`, `justificar_retraso`, `enlazar_bitacora_tarea`, `recalcular_cronograma`, `puede_ver/gestionar_cronograma`; tablas `cronograma_tareas`/`_recalculos`/`_tarea_bitacoras`; bucket `sgc-cronograma`; avisos `avisos_proyecto` (tipos cronograma_*) + edge `notificar-cronograma`.
