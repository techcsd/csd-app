...
**TL;DR:** (1) Con OK de Xaviel, **2.28.0 (BW) salió a PROD**. (2) Ronda **BX** en la app: **F1 no-op** (la app no muestra historial de edición de echada), **F2 = rol Developer** (routing por rol). `npm run build:dev` **verde**. (3) Con OK ("probé en dev, ok"), **2.29.0 (BX) TAMBIÉN salió a PROD**: merge `dev→main` (PWA) + APK prod firmado/registrado/subido + **publicada=2.29.0** (mínima sigue 2.26.1). ✅ **Sesión cerrada — nada pendiente de release.**
...
### ✅ En PROD (RELEASE 2.29.0 — con OK "probé en dev, ok")
- **`dev→main`** mergeado + pusheado (`33049a4`) → Vercel construye la PWA prod.
- **APK prod 2.29.0**: firmado (cert `3c5316d8…`), regla 18 OK, registrado en `app_versiones` prod + subido al bucket prod.
- **PUBLICADA = 2.29.0 · MÍNIMA = 2.26.1** (verificado: `version_publicada()` → pub 2.29.0 / min 2.26.1 / apk `csd-app-2.29.0.apk`).

### ✅ En dev (lo que se construyó antes del release)
...
### ⏳ Owed físico de Xaviel (post-release)
- **Aplicar `sql-para-sgc/2026-09-23-bx1b`** (padre, dev→prod) para que el Developer vea errores/marque versión. Cuando viva en dev, el hijo añade `'desarrollador'` a `UserContextService.TECNOLOGIA` (1 línea) y activa la pestaña de errores.
...
- **✅ Release 2.26.1 hecho:** APK publicado (no mínima) + PWA a main desplegada (ver bloque de arriba). El inglés al 96% (sin beta) ya llega a Android (opcional) e iPhone (Vercel).
...
- **Rollback:** en SGC `update sgc.app_versiones set minima=(version='2.26.0'), publicada=(version in ('2.26.0','2.25.0','2.24.1')) where plataforma='movil'` (vuelve pub+mínima a 2.26.0) + `git revert` de los commits de esta sesión y `git push origin HEAD:main` (revierte la PWA). El APK 2.26.1 queda en el bucket pero deja de ser pub/mínima. No toca RPCs ni esquema.
...
- **Device-QA de 2.24.1 (Redmi Note 10 Pro, 17-sep) — COMPLETA:** **F4 rechazar** ✓ (el link "Hay un problema — rechazar esta entrega" y el panel motivo-obligatorio+foto salen; el guard bloquea envío con motivo vacío → toast "Escribe el motivo del rechazo (es obligatorio)"; cancelado, sin mutar el conduce real). **F1 combustible salto de km** ✓ (Izuzu D-Max PRUEBA, última echada 49,000 → km 55,000 = salto 6,000: apareció el aviso **no bloqueante** "Han pasado 6000 km desde la última echada registrada; se enviará para revisión de Logística" y **AVANZÓ** al paso 3, no bloqueó; descartado → 0 echadas creadas). El resto ya estaba device-verificado en 2.24.0 (cartillas E2E `CAR-000001`, idioma en vivo). **Toda la ronda BR queda device-verificada.**
...
**🔴 Felix (BQ7) — CERRADO server-side:** aplicado a prod + **commiteado/pusheado en SGC** (`421faad`, `sql/2026-09-15-bq7b-combustible-af18-uso-v2.sql`): el gate **AF18** de `registrar_combustible_app` ahora **une la sesión uso-v2** (`vehiculo_usos.fin_at is null`) al chequeo de tenencia, no solo `vehiculo_asignaciones`/`responsable_id`. Aditivo (solo afloja, valida begin/rollback, overload único + grant intactos). Un chofer que RECIBIÓ el vehículo por uso-v2 ya puede echarle gasolina (fin del DR481 sistémico). ⚠️ Los **3 payloads viejos de Felix** drenan al **Reintentar** solo si esos vehículos ahora están sin asignación a otro **o** los tiene en uso; su vehículo/uso concreto depende de su teléfono (device). Diagnóstico completo en memoria `bq7-af18-gate-server-uso-blind`.

**2.22.0 → 2.23.0 acumulado** (todo lo de esta sesión): F1.5 barra con tipo/deep-link · BQ6+BL9 fecha local (6 payloads) · F3 molde-esquema v2 (formas) · **F4 compositor** (opt-in) · **F3.2 multi-lado** · BQ2 texto correo · BQ4 verificado · BQ10 deep-link Compa · **BQ7b AF18 (SGC)**.

⚠️ **Sin device-QA formal** (como 2.21.0/2.22.0). Riesgo contenido: compositor y multi-lado son **aditivos/opt-in**, el flujo Ficha de siempre queda intacto. **Rollback app**: SGC `update sgc.app_versiones set publicada=(version='2.22.0'), minima=(version='2.22.0') where plataforma='movil' and version in ('2.22.0','2.23.0')`; PWA revert de los commits de la sesión. **Rollback AF18**: reaplicar el def previo del RPC (era idéntico salvo la condición del raise).

**🪞 Paridad web (regla 5) — SGC 1.132.0:** el **compositor** se **mirroró en la web** (`SGC/src/app/pages/bitacora/nueva` — toggle "Ficha / Compositor" en "Moldes del día", `figuraToMoldeRow`, `desdeCompositor`, borrador con `moldeModo`+`compositorFiguras`). El componente `molde-compositor` ya existía en `SGC/shared/ui` pero **no estaba integrado**. **Multi-lado ("+ Agregar lado") YA estaba en la web** (la app iba detrás; se cerró la paridad esta sesión con F3.2). Commit SGC `df6511b`, pusheado a main → Vercel; **web 1.132.0 registrada (Y1)** con su cambio estructurado. SGC build + verify-tokens verdes.

**Verify on resume:** `select version,publicada,minima from sgc.app_versiones where plataforma='movil' order by created_at desc limit 3` → 2.23.0 pub+min. **Device-QA pendiente** (foco: compositor táctil, formas L/T/U/circular, "+ Agregar lado", y que un chofer con uso-v2 registre combustible). Nada más pendiente app-side de la ronda BQ; paridad web cerrada (compositor + multi-lado + AF18).

**TL;DR:** la mayoría de PROMPT-51 depende de contratos del PADRE (SGC) aún por desplegar → introspección de prod antes de tocar. Lo que SÍ se hizo, verificado y compilando: **(1) diagnóstico completo de BQ7** con un hallazgo que cambia el plan; **(2) FASE 1.5** — la barra roja dice QUÉ (`"3 echadas de combustible con problema"`) y al tocarla va DIRECTO al primer envío con problema (`/pendientes/<id>`, outbox-detalle), no a la lista; **(3) BQ6 (mitad app)** — el bug de la inspección que "no sale en la semanal" ERA de la app: `reporte-semanal.ts:752` mandaba `new Date().toISOString().slice(0,10)` (día UTC) → una inspección del domingo 21:00 caía al lunes → semana siguiente. Arreglado a `fechaLocalISO()` (BL9). El RPC `registrar_checklist_vehiculo` usa `coalesce(p_fecha, current_date)` → **respeta el día que manda la app**, así que el fix es completo sin tocar el padre. **(4) Barrido BL9** del mismo antipatrón en 5 payloads más (combustible, preuso, mantenimiento, crear-ruta, tareas iniciar/completar). `npm run build` + `verify-tokens` verdes. **HEAD sin cambios (pendiente de autorización).**

### 🔴 BQ7 — Felix no puede registrar combustible: DIAGNÓSTICO (la causa es del servidor + data, NO de la app)
Introspección de prod (Management API, read-only):
- **Dos Felix distintos**: el chofer de BQ7 = **Felix Manuel Mateo Sanchez** (usuario `ace53024…`, conductor `22f40493…`). *FELIX DELGADO* (`2ffde1d1…`) es el de Incentivos (captura 9), otra persona.
- **La sobrecarga viva de `registrar_combustible_app`**: UNA sola (20 args), `has_function_privilege('authenticated')=true`. **Hipótesis 2 y 3 del CONTEXTO (grant/ambigüedad 42501/42725) DESCARTADAS.** La firma de la app coincide exacto (regla 11 ✓).
- 🟢 **El padre YA desplegó el "BQ7 blindaje de conductor_id"** (F1.4): el cuerpo vivo ignora un `p_conductor_id` que no exista (fusionado/borrado por BP1 → `raise notice`), lo resuelve por `auth.uid()` (`asegurar_mi_conductor`) y cae a la asignación del vehículo. → El id fantasma en los payloads congelados de Felix **ya no rompe el INSERT** (hipótesis 1 / 23503 neutralizada server-side). *"Lo bloqueado ya estaba desplegado"* otra vez — verificado, no asumido.
- 🔴 **PERO la causa REAL sigue viva y es del padre**: el gate **AF18** (`if not es_flota_elevado() then … 'Solo el usuario asignado a este vehículo puede registrar su combustible' using errcode='DR481'`) **solo mira `vehiculo_asignaciones` + `vehiculos.responsable_id` — NO une la sesión uso-v2 (`vehiculo_usos`)**. Es el MISMO fallo que BO2 (memoria `bo2-vehiculo-mine-gate-uso-union` / `fuel-rights-follow-en-uso`), pero **server-side**. Telemetría lo confirma: `app_error_reports` muestra `[combustible] Solo el usuario asignado…` recurrente en 2.13/2.18/2.19.1/2.20.0.
- **Estado de Felix en prod**: 0 asignaciones activas, 0 usos (nunca), 1 ficha de conductor limpia (BP1 mergeó sin dejar huérfanos — `dangling assignments/usos = 0`). Su única echada exitosa (07/09) la registró OTRO usuario (`7c7ffd84…`), que sí es elevado y saltó AF18.
- **DR481 se clasifica 'dato' (negocio), NO 'sistema'** (throwSyncError→'validacion', outboxCategoria→'dato'; la app lo pinta *"Revisar dato"*, sin Corregir por ser DR*). La inferencia del CONTEXTO de que es "sistema" viene de leer el texto de la barra (`"N con problema"`), no la tarjeta.
- **Felix NO aparece en `app_error_reports` ni en `outbox_atascados`** → sus 3 SQLSTATEs siguen SOLO en su teléfono (F1.1 tenía razón: hay que copiarlos del device). Pero el diagnóstico ya no depende de eso.

**Conclusión BQ7 → esto es del PADRE (PROMPT-50 F1) + data, la app no puede arreglar la autorización:**
1. **Fix AF18** para unir uso-v2 (y/o el propio `conductores.usuario_id` del registrante) — como BO2 pero en el RPC. Sin esto, el reintento de Felix vuelve a DR481 si el vehículo del payload tiene asignación/responsable a otro.
2. **Re-vincular a Felix** a su(s) vehículo(s) (asignación o uso), o confirmar que los vehículos de los 3 payloads quedaron sin asignación/responsable → entonces el reintento pasa tal cual.
3. Con (1)/(2) hechos: Felix toca **"Reintentar todos"** una vez → drenan **conservando las fotos**. **§F-2 "Reenviar como nuevo" NO hace falta** (los payloads son válidos; un `retry` normal basta cuando el servidor los acepte). Recomendación: no construirlo.

### 🟢 FASE 1.5 — barra "N con problema" (app, HECHO)
- `outbox-labels.ts`: nuevo `tipoOpNoun(t,n)` (singular/plural para los tipos de captura de campo; fallback `envío(s)`).
- `sync.service.ts`: nuevos signals `errorFirstId` (primer error FIFO) + `errorTipos` (Map tipo→conteo), poblados en `refreshCounts` (que ya cargaba el outbox — costo cero).
- `sync-bar.ts`: texto enriquecido — si todos los errores son de un tipo → `"3 echadas de combustible con problema · toca para revisar"`; `abrir()` va a `/pendientes/<primerError>` (outbox-detalle: contenido + error + Reintentar/Duplicar/Descartar) cuando hay errores. Desaparece sola por `errorCount`/`sync.changed` (verificado en código).

### 🟢 FASE 2 (BQ6, mitad app) — fecha local (HECHO)
- `reporte-semanal.ts:752` `new Date().toISOString().slice(0,10)` → **`fechaLocalISO()`**. RPC vivo respeta `p_fecha` (`coalesce(p_fecha,current_date)`) → fix completo app-side. La otra mitad (scoping T7 / es_prueba de la web, o el correo de operaciones) es del padre (§F-4).

### 🟢 Barrido BL9 (mismo antipatrón en payloads, HECHO)
Todos día-UTC → `fechaLocalISO()`: `combustible.ts:716` (fecha de echada), `preuso.ts:671` (misma tabla checklists_vehiculo), `mantenimiento.ts:316`, `crear-ruta.ts:786`, `tareas.service.ts` (`fecha_inicio`/`fecha_fin`). Substitución probada-segura (el día local nunca es peor que el UTC). Displays/filtros con el patrón (`obra/*.hoy`, `informe` week bounds, xlsx, auditoría) NO tocados (no son data de registro).

### 🟢 SEGUNDA TANDA (mismo turno) — el padre YA desplegó casi todo (verificado en prod + repo SGC 1.131.0) → se destrabó mucho:
- 🟢 **F3 BQ8(a) — molde-esquema v2 (el bug de las 5 formas)**: el padre lo shipeó (`SGC/src/shared/ui/molde-esquema`, 415 líneas). **Copiado VERBATIM** (.ts + .html) a `src/app/shared/ui/molde-esquema/` (+`standalone:true`); `.scss` ya era de tokens y cubre todas las clases v2 → sin cambio. Ahora dibuja por `forma` (rectangular/L/T/U/circular) y compone multi-tramo; `derive()` arma L/T/U desde UN tramo (largo→A, alto→B, espesor) → **el bug reportado ("elija la forma que elija, sale el mismo rectángulo") queda arreglado** sin depender de captura multi-lado. Misma API (`forma/tramos/medidaPlano/toleranciaCm`) → los 3 consumidores (parte preview, parte lista, detalle) funcionan sin tocar. **F3.3 ya estaba** (detalle pinta `identificador · estructura`, `detalle.html:177`).
- 🟢 **F5 BQ2 — texto de *Preferencias de avisos*** (`avisos.html:70`): el padre desplegó `destinatarios_notificacion` (web 1.130.0, "correo respeta silencios") → la leyenda dejó de mentir. Antes: *"…dejan de llegarte (aquí y como notificación)"*. Ahora: *"…por todas las vías: aquí, como notificación y por correo. Algunos, por ser críticos o de Administración, siguen llegando."*
- 🟢 **F6 BQ4 — rol `encargado_patio` VERIFICADO**: existe en prod con `modulos:['inventario']` **+ permisos granulares** `{proyectos.personal:operar, rrhh.asistencia:operar}` (Xaviel siguió el §F-5 completo). La app lo gatea bien: Inventario por `hasModulo('inventario')`; Personal de obra por `puedeVerSubmodulo('proyectos.personal')` (home.ts:232, ingenieria.ts:78, proyecto-detalle.ts:116) — `nivelSubmodulo` honra el grant granular SIN el módulo padre. Rutas `proyectos/personal/*` = auth+pin (RLS-scoped). **Smoke pendiente**: el usuario de El flaco aún no existe (lo crea Xaviel, §F-5); si entra por cédula+PIN, ojo que `acceso-cedula` es chofer-only (PADRE). No se halló hardcode cédula→Transporte en el login (role-driven).
- 🟢 **F7 BQ10 — deep-link `compa_capacidad_nueva`**: `notifAppRoute` (`notificaciones.service.ts`) mapea `tipo='compa_capacidad_nueva'` / ruta web `/asistente?q=…` → **`/compa?q=<pregunta>`** (decode+encode robusto). `CompaPage` ahora lee `?q=` y **precarga la pregunta** en el composer (override del borrador; es intención directa). Forward-compatible: funciona cuando el padre empiece a emitir la notif.

- 🟢 **F4 BQ8(c) — compositor de moldes (PORTADO + INTEGRADO, opt-in)**: copiado del padre (`molde-compositor.ts` verbatim +`standalone:true`; `.html` sin emojis; `.scss` a tokens `--color-*` + targets táctiles: paleta ≥56px, inputs 48px, teclado numérico `inputmode="decimal"`). Integrado en el **paso 10** como segmentado **Ficha / Compositor** (default **Ficha**, seguro): en modo compositor cada figura = un molde (`figuraToMolde`, key `fig_*`), escribe la MISMA lista `moldes` (payload sin cambio) conservando las filas de la ficha; borrador persiste `moldeModo` + `compositorFiguras` (rehidrata vía `valorInicial`). ⚠️ **UX táctil SIN device-QA** — es opt-in y el flujo Ficha queda intacto, así que el riesgo está contenido; **validar drag/resize/snap en device** antes de promocionarlo. El padre construyó el componente pero **no lo integró** en su captura (la app es ahora la referencia de integración — espejo pendiente en SGC).

### ⏸ Sigue bloqueado (device / decisión de Xaviel)
- **F0 device-QA de 2.21.0 + 2.22.0** — requiere device/PWA físico (sesión de fondo, sin ADB). **Primero al volver con device**, con foco en el compositor (F4) y las formas de moldes (F3).
- **F3.2 multi-tramo ("+ Agregar lado" por molde)** — DIFERIDO: cambia el modelo `MoldeRow` (tramos[]) + payload; v2 ya dibuja L/T/U desde un tramo y el compositor cubre la composición multi-figura. Refinamiento para device-QA.

**Verify on resume:** `node scripts/verify-tokens.mjs` verde (baseline 88); `npm run build` verde. Probar en device: (a) forzar un error de combustible → la barra dice *"1 echada de combustible con problema"* y al tocar abre el detalle de ESE envío; (b) capturar una inspección tras las 20:00 (RD) → la vista de cumplimiento la cuenta en la semana correcta; (c) en el paso 10 del parte, elegir forma **L/T/U/circular** → el esquema dibuja ESA forma (no un rectángulo); (d) abrir `/compa?q=hola` → el composer trae "hola" precargado; (e) *Avisos → Preferencias* → la leyenda menciona el correo. **HEAD sin cambios commiteados (gateado a Xaviel).** Introspección: scripts read-only en el scratchpad de la sesión (no versionados). **Archivos tocados esta sesión:** `sync.service.ts`, `sync-bar.ts`, `outbox-labels.ts`, `reporte-semanal.ts`, `combustible.ts`, `preuso.ts`, `mantenimiento.ts`, `crear-ruta.ts`, `tareas.service.ts`, `molde-esquema.ts/.html`, `notificaciones.service.ts`, `compa.ts`, `avisos.html`.

---

## 🟢 SESIÓN 14/09/2026 (tarde) — PROMPT-49 ronda BP — **daños + moldes + Dev notes + BM5 + BL2 → RELEASE 2.21.0 PUBLICADA + MÍNIMA FORZADA** · pusheado a main · build+tokens verde · review adversarial (2 fixes)

**RELEASE 2.21.0 (14-sep):** bump 2.20.0→2.21.0 (environments + build.gradle + release-apk VERSION/MIN_VERSION/CAMBIOS_CURADOS). Commit `7358e0b` pusheado a `main` (PWA→Vercel). **APK firmado** (cert prod `3c5316d8…5065`, v1+v2+v3), **registrado Y1** (4 cambios curados) y **subido al bucket** (`csd-app-2.21.0.apk` + `latest` + `version.json`). **PUBLICADA + MÍNIMA = última**: `app_versiones` 2.21.0 `publicada=true, minima=true`; **minima de 2.20.0 limpiada** (solo la última fuerza). Verificado: `sgc.version_publicada()` → `version_publicada=2.21.0`, `version_minima=2.21.0`, apk_url ok. ⚠️ **Sin device-QA formal** (Xaviel pidió release directo tras el build) — los pasos daños/moldes son aditivos y saltables (Sí/No), el flujo existente quedó intacto; **rollback** si algo falla: en SGC poner `minima=true` de vuelta en 2.20.0 (o `publicada=false` en 2.21.0).

**Verify on resume del release:** `select version,publicada,minima from sgc.app_versiones where plataforma='movil' order by created_at desc limit 3` → 2.21.0 pub+min, 2.20.0 pub sin min. Historial web "Versiones de la app" muestra 2.21.0 con sus chips.

---

## 🟢 SESIÓN 14/09/2026 (tarde) — PROMPT-49 ronda BP — detalle técnico (build)

**TL;DR:** el prompt asumía F2/F3/F4 ⏸ *bloqueadas por el padre*; **introspección de prod (Management API) probó lo contrario: todo está desplegado** → se invierte el plan (ver memoria `bp4-bp5-bo9-contracts-deployed`). Hecho y verificado a nivel build: **F0.3 (BM5)** hueco cerrado, **F1 (BP2)** auditado (la app NO tiene el bug), **F0.4 (TECNOLOGIA)** espejo verificado = coincide con prod, **F4 (BP5 Dev notes lectura)**, **F2 (BP4 daños)** y **F3 (BO9 moldes)** CONSTRUIDOS end-to-end (captura paso 9 daños + paso 10 moldes con esquema SVG en vivo + outbox → `guardar_bitacora_extra` + detalle + mis-partes). El wizard del parte pasó de 10 a **12 pasos**. Todo con `npm run build` verde + verify-tokens verde; FKs/RLS/grants/param verificados en prod (los embeds no rompen la lista). **Review adversarial del path daños/moldes** halló y arregló 2 bugs (`14fecb3`): (1) borrador recuperado + daño con retiro sin foto revalidado en `submit()` (evita bitácora a medias); (2) "tengo plano" a medias marcaba falsa desviación → el plano solo cuenta con largo+espesor. **PUSHEADO a `main`** (Xaviel autorizó): `20fdd7a` (feature) + `14fecb3` (fixes) + `6e48ce6` (docs) + `3f4dd4a` (BL2 mis-nc) → **PWA se despliega a Vercel**. **Falta device-QA offline/online** (recorrer los 12 pasos en el device/PWA). **APK release (build + Y1 + publicar + mínima) NO hecho** — sigue gateado a Xaviel; la app nativa sigue en 2.20.0 hasta que se genere y publique el APK. F5 (BP3) = verify-only. F6 (cartillas) = solo nota.

**Backlog tocado esta sesión:** ✅ **BL2 mis-nc** (`obra/mis-nc` distinguía mal "sin señal" de "todo al día" — `cargar()` sin catch; arreglado + Reintentar, `3f4dd4a`). **NO tocado (con motivo):** `esChofer` mirror (`user-context.service.ts:146`) — el server `sgc.es_chofer()` = `incentivo_participante.es_chofer` OR rol; mirror-arreglo limpio sería llamar el RPC `es_chofer()` al cargar perfil (con fallback rol offline), pero es un cambio de GATING (Tecnología/Mi rendimiento) de riesgo en prod sin device-QA y valor nicho → dejar para device. `home` "Sin módulos asignados" — deriva del perfil global (cacheado offline); un falso-positivo requiere sesión sin perfil cacheado; añadir estado de error arriesga el boot → dejar. `visita` screen (paridad), BM5 app-qty-input (decisión) — sin tocar.

### ✅ F0 — Verificación de lo repetido (§C) + huecos
- **2.20.0** publicada + mínima (prod `app_versiones`). BO2/BO4/BO5/BO8 confirmados en código (`FLOTA_ELEVADO` con `logistica`, `esFlotaElevado`, `fechaNecesidad`). *"Ya en prod desde 2.20.0."*
- **F0.3 — BM5 hueco CERRADO** (`shared/ui/selector-categorias/selector-categorias.html`): la copia de **búsqueda de la hoja de categorías** (líneas ~140-169) no tenía el toggle atado/unidad y usaba `cantidadDe(a.id)`. Ahora **espeja las otras dos copias** (`soloBuscador` + `hoja='categoria'`): añadido el bloque `@if (puedeEmpaque(a))` + qty con `[ngModel]="cantidadMostrada(a)"`. Data-safe (captura SIEMPRE en base + `unidad_capturada`/`factor_aplicado`). Métodos ya existían en el `.ts`.
- **F0.4 — espejo TECNOLOGIA verificado** (regla 12 corolario): prod `sgc.es_tecnologia()` = `admin, tecnologia, gerencia, direccion` → **coincide exacto** con `UserContextService.TECNOLOGIA`. El CONTEXTO-23 decía "admin|tecnologia" (stale). No hay drift.

### ✅ F1 — BP2 (nota vacía): la app NO tiene el bug
- El editor de la app (`pages/notas/editor/nota-editor.ts:118-124`) YA usa el arreglo prescrito: `effect()` sobre el `viewChild` gateado por `!loading()` + flag `bodyHidratado`, **no** un `queueMicrotask` que corre antes de que el `<div>` exista. Sin autosave-por-tecla (guarda al salir), sin riesgo de pérdida. Único otro `queueMicrotask` del repo = `qty-input.ts:75` (`el.select()`, elemento por argumento, no bajo `@if`) — seguro. **Nada que arreglar.**

### ✅ F4 — BP5 Dev notes (SOLO LECTURA en la app)
- Contrato del padre desplegado: `sgc.notas` con `ambito/formato/tags` + `guardar_nota` acepta `p_ambito/p_formato/p_tags` (verificado en prod). RLS limita `ambito='dev'` a `es_tecnologia()`.
- **Nuevo**: `pages/tecnologia/dev-notes/` (lista mis/compartidas + búsqueda + filtro por tag + lector Markdown a pantalla completa + botón "Editar en SGC web" deep-link `sgcconstructorasd.com/tecnologia/dev-notes/<id>`). Ruta `/tecnologia/dev-notes` (authGuard+pinGuard, gate `esTecnologia()` dentro del componente + RLS backstop). Tile "Dev notes" en `tecnologia.html` solo para `esTecnologia()`.
- **Render Markdown**: componente compartido nuevo `shared/ui/markdown-view` con **`marked ^18.0.13` + `dompurify ^3.4.15`** (mismas versiones que el padre). **Sin `highlight.js`** (decisión: peso del APK; bloques de código monoespaciados con **scroll horizontal**, tokens BD3). marked/dompurify quedan en el **chunk lazy** de dev-notes (NO en el bundle inicial — verificado: `main-*.js` no los referencia).
- `notas.service.ts`: `ambito/formato/tags` añadidos al `select` + `mapNota` (retrocompat: viejas → general/html/[]). `/notas` (lista general) ahora **filtra `ambito='dev'`** fuera (F4.4). Modelo `Nota` extendido.

### ✅ F5 — BP3 (logistica → proyectos): verify-only, sin código
- Prod: `sgc.roles` codigo `logistica` `modulos` YA incluye `'proyectos'` (el padre aplicó la migración). Raykler + **Misael** lo ganan (avisado en CONTEXTO). La app gatea por módulo (`hasModulo('proyectos')` / `puedeGestionarProyectos` espejo que excluye `ingeniero_oficina`) → el tile Proyectos y "Nuevo proyecto" se encienden solos tras refresco de perfil. **No hay gate por lista-de-roles** para proyectos en la app (grep confirmado). ⚠️ La app cachea el perfil: Raykler necesita **re-login o esperar `ensureFreshProfile` (5 min)**.

### ✅ F2 — BP4 daños en la bitácora (CONSTRUIDO end-to-end, falta device-QA)
- **Contrato**: `sgc.guardar_bitacora_extra(p_bitacora_id, p_extra jsonb)` con `p_extra={danos:[...],moldes:[]}`, **replace idempotente** por bitacora_id. Se llama en el MISMO handler `bitacora` **DESPUÉS** de `crear_bitacora_app` (que es idempotente por p_id) — sin nuevo `tipo_op`. Si hay 0 daños no se llama.
- **Captura** (`parte.ts/.html`): **paso 9 nuevo** ("¿Se dañó algún material o equipo propio hoy?"), ingeniero→10, resumen→11, `TOTAL 10→11` (step-bar por `total()`). Sí/No + toggle tipo (Material/Equipo propio). Material = `app-articulo-picker` (soloBuscador + buscador fuzzy) **o** "No está en la lista"→texto libre (`nombre_libre`). Equipo propio = texto libre (**`sgc.equipo_obra` NO existe** → sin selector de equipo). Cada daño: `app-qty-input` (cantidad en base; `unidad_capturada=unidad`, `factor_aplicado=1` — el toggle atado BM5 se puede sumar después, aditivo) + unidad + detalle (obligatorio) + fotos (`photo-slot`) + (solo material) "Solicitar retiro a almacén" con aviso "necesita ≥1 foto".
- **Payload/handler** (`bitacora.service.ts`): `ParteDiarioCaptura.danos`, payload campo por campo (regla 10), fotos por slots `bdano_<i>_<j>` (`buildDanoBitacoraFotos`, excluidas del montón general), resueltas a `fotos_paths` en el handler. Validación cliente en `next()` (paso 9) espeja el RPC (detalle req, material+retiro→≥1 foto) para no atascar el outbox con 22023.
- **Lectura**: `BITA_SELECT` embebe `danos:bitacora_danos(...,articulo:articulos(nombre),retiro:retiros_material(folio))`; **FKs + RLS + grant `authenticated` verificados en prod** (el embed NO rompe la lista). `detalle` pinta sección **"Daños"** (tipo, cantidad, detalle, fotos firmadas, folio **RET-** clicable → `/inventario/retiros`); `mis-partes` muestra chip "🧱 N daños".
- **Draft**: persiste `huboDanos` + filas `danos` (sin fotos, se retoman — convención del parte). Un retiro rehidratado sin foto lo caza la validación del paso 9.
- ⚠️ **Falta device-QA**: probar offline→avión→volver→drena + verificar en SGC web (parte con sección Daños, RET- en Retiros). No se pudo en sesión remota.

### ✅ F3 — BO9 moldes (CONSTRUIDO end-to-end, falta device-QA)
- **Contrato**: mismo `guardar_bitacora_extra` (`p_extra.moldes`). Server calcula `desviacion_max_cm` (real↔plano por posición) + notifica `>molde_tolerancia_cm` (prod=2). FK/RLS/grant de `bitacora_molde_medidas` **verificados en prod** (embed seguro).
- **Componente SVG**: `shared/ui/molde-esquema` **copiado exacto del padre** (.ts/.html verbatim; .scss adaptado a tokens `--color-*`, cero hex, texto ≥14px, trazo real 2.5px). Dibuja rectángulo real + plano punteado + cotas; rojo si fuera de tolerancia. Nota: como el padre, el esquema usa `tramos[0]` (rectángulo base) para las 5 formas.
- **Captura** (`parte.ts/.html`): **paso 10 nuevo** ("¿Trabajaste moldes hoy?", auto-Sí si alguna actividad es ENCOFRADO), ingeniero→11, resumen→12, `TOTAL 11→12`. Formulario por molde: estructura (catálogo), identificador, forma (5 `option-button`), medidas reales largo/alto/espesor (cm, teclado numérico), "Tengo la medida del plano"→mismos campos, **esquema en vivo** (`app-molde-esquema`), nota, foto opcional → "+ Agregar molde" → lista con mini-esquema. Simplificación: **un tramo por molde** (largo/alto/espesor) — es lo que el esquema del padre renderiza; multi-lado es extensión aditiva futura.
- **Payload/handler**: `moldes` campo por campo, fotos por slots `mfoto_<i>_<j>`, mismo `guardar_bitacora_extra` (ahora `{danos, moldes}`; se llama si hay daños **o** moldes).
- **Lectura**: `detalle` pinta sección "Moldes" con el esquema por molde + badge **"Desvío X cm"** si `desviacion_max_cm > 2` + fotos firmadas; `mis-partes` chip "📐 N moldes".

### 🟡 F6 — BO10 cartillas: nada en la app esta tanda (como pide el prompt)
- Esperar `docs/CARTILLAS-PROPUESTA.md` validada por Guilamo/Ramón antes de construir. Molde de arranque cuando toque: `retiros.service.ts` (BG4, outbox idempotente por client_id) + fecha elegible BL9 (`bitacora.service.ts` patrón `fecha`/`fechaLocalISO`) + `tipo_op:'cartilla'`. El padre redacta la propuesta y se detiene (PROMPT-48 F8).
- **F6 — cartillas (BO10)**: nada en la app esta tanda. Esperar `docs/CARTILLAS-PROPUESTA.md` validada; molde = `retiros.service.ts` (BG4) + fecha BL9 + `tipo_op:'cartilla'`.

**Verify on resume:** `node scripts/verify-tokens.mjs` → verde (baseline 88). `npm run build` → verde (warnings pre-existentes de budget/NG8102/NG8107). **F2 daños** (paso 9) + **F3 moldes** (paso 10): Bitácora → Nueva → recorrer los 12 pasos; en daños agregar material+equipo propio con detalle/foto/retiro; en moldes decir "Sí" (auto si hubo ENCOFRADO), llenar largo/espesor → ver el **esquema en vivo**, marcar "tengo plano" con una desviación >2cm → ver la cota en rojo → Agregar → enviar → en detalle salen "Daños" (RET- en `/inventario/retiros`) y "Moldes" (esquema + "Desvío X cm"); **device-QA offline/online pendiente**. **F4 Dev notes**: login rol tecnología → tile "Dev notes" → lista + lector Markdown; `/notas` normal NO muestra `ambito='dev'`. **BM5**: buscar artículo `factor_paquete>1` en el buscador de la hoja de categorías → toggle atado/unidad. **HEAD sin cambios nuevos commiteados** (pendiente de autorización de Xaviel).
...
- **Publicada + MÍNIMA FORZADA a la última** (política de Xaviel: **la mínima = la última actualización**). PATCH service_role: mínima limpiada de 2.16.0 → **`minima=true` en 2.19.0**. Gate verificado: `version_publicada()` = **`{version_publicada:2.19.0, version_minima:2.19.0, apk_url:…2.19.0.apk}`**. Todos los APK < 2019000 quedan **forzados a actualizar**; el PWA/iPhone se autoactualiza por SW (la mínima solo bloquea la APK nativa).
- **Verificado E2E (no solo razonado):** gate → 2.19.0; la **web NO se rompió** con la migración BN1b (llamada web de 13 args a `crear_orden_trabajo` resuelve → P0001 "No autenticado", no PGRST202); la app 14-args (con p_id) también resuelve.
- **Rollback Android**: `update sgc.app_versiones set publicada=(version='2.16.0'), minima=(version='2.16.0') where plataforma='movil';` · **Rollback PWA**: `git revert d6431e7 && git push`.

### ✅ SGC repo — pusheado
- `git push origin main` en `../dev/SGC` (`028765d..6a05091`): incluye BN1b (idempotencia) + el commit previo `3024612 BN3 web 1.127.2`. La web desplegará. La migración BN1b ya estaba aplicada a prod desde antes del push.

### Nota sobre la política de mínima (corrige HANDOFF previos)
- Los HANDOFF 2.14–2.18 decían "mínima intacta 2.13.0" — **eso era un malentendido**. La política real de Xaviel es **mínima = la última versión publicada** (forzar a todos a la más nueva). Por eso 2.16.0 estaba como mínima. Cada release debe mover la mínima a sí mismo. Memoria [[release-minima-and-selfpin]] corregida.
...
- **Commits `723512e` (feat BF) + `c37d621` + `<este>` en `main`, PUSHEADOS** → Vercel deploya la PWA 2.9.0 (iPhone/web). Versión alineada en environment(.prod).ts / build.gradle / release-apk.mjs.
- **APK 2.9.0 firmado** (cert prod `3c5316d8…5065`), **registrado (Y1)** con `CAMBIOS_CURADOS` (conduces/requisiciones/personal/avisos) y **subido al bucket** (`csd-app-2.9.0.apk` + `latest` + `version.json` + `apk_url`). Descarga: `…/app-releases/csd-app-2.9.0.apk`.
- **Publicada:** `sql/2026-09-01-publicar-2.9.0.sql` aplicada → `publicada=(version='2.9.0')` para móvil (rolling a todos). Verificado: 2.9.0 publicada=true; **mínima = 2.8.1** (NO la toqué — es tu dominio admin; anota que ya no es 1.96.4). **Rollback Android:** `update sgc.app_versiones set publicada=(version='2.8.1') where plataforma='movil';` **Rollback PWA:** `git revert 723512e && git push`.
...
### 🚀 Estado / release 2.7.0 — PUBLICADA (rolling)
- **Commit `e9b14f9` + push a `main`** → Vercel deploya la **PWA 2.7.0** (iPhone/web). Versión bumpeada a 2.7.0 en environments/build.gradle/release-apk.mjs.
- **APK 2.7.0 firmado** (cert prod `3c5316d8…5065`), **registrado (Y1)** y **subido al bucket** (`csd-app-2.7.0.apk` + `latest` + `version.json` + `apk_url`). Descarga: `…/app-releases/csd-app-2.7.0.apk`.
- **Publicada:** `sql/2026-08-31-publicar-2.7.0.sql` aplicada → `publicada=(version='2.7.0')` para móvil (rolling a todos). **Rollback:** `update sgc.app_versiones set publicada=(version='2.6.0') where plataforma='movil';` **Rollback PWA:** `git revert <commits> && git push`.
- ⚠️ Observación (NO la toqué, es tu dominio admin): en `sgc.app_versiones` móvil hay **DOS filas con `minima=true`: 1.96.4 Y 2.6.0**. La mínima efectiva sería 2.6.0 (fuerza a quien esté por debajo). Si esperabas solo 1.96.4, límpialo (`update … set minima=(version='1.96.4') where plataforma='movil';`).
...
- **✅ Asignaciones alineadas a la realidad (a pedido de Xaviel):** `sql/2026-08-29-alinear-asignaciones-a-uso.sql` (APLICADA). Corrección **de una vez** (NO auto-sync): cada chofer con uso activo quedó asignado al vehículo que USA. Cambios: Manolo↔Polin estaban CRUZADOS (Manolo asignado Nissan/usa KIA; Polin asignado KIA/usa Nissan) → ahora Manolo→KIA L473027, Polin→Nissan L441660; Misael AB2890340→L478815; Edward Mota (sin asignación)→MT 03. Joan y Test User 3 ya coincidían. Patrón traspaso (desactivar vieja + insertar nueva, una activa por vehículo + `responsable_id`); reversible (viejas quedan `activa=false`/`hasta`). Verificado: todos "ya coincide", ningún vehículo con >1 asignación activa. La web ve el dato corregido automáticamente (misma BD).
