...
## Ronda 6 app — PROMPT-14 / Act.5 (2026-07-22) — v1.24.0 (U1–U16)
Source: `C:\developer\improvements\imp 20072026\CONTEXTO-ACTUALIZACION-5.md` (U1–U16) + `PROMPT-14-CSD-APP.md`. `npm run build` VERDE por fase y final (0 errores; 13 warnings NG8102 pre-existentes). **PROMPT-13-SGC ya aplicado** en la BD compartida (verificado: `PRE-USO-V4` y `REPORTE-SEMANAL-V3` activas con etiquetas cortas de 9 secciones×1 ítem). Commit+push a `main` + APK 1.24.0 firmado + subido al bucket + historial registrado (Y1, 12 cambios). **NO marcada publicada/mínima** (paso admin en SGC, tras tu QA).
...
- **PENDIENTE device-QA (necesita teléfono/JWT real; APK 1.24.0 + PWA iOS):**
  ...
  - [ ] **Marcar 1.24.0 publicada/mínima en SGC** tras QA (hoy quedó registrada pero NO publicada).
