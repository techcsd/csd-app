...
## Ronda 29/07 — PROMPT-10-CSD-APP (IDs AA, CONTEXTO-ACTUALIZACION-2) — 2026-07-29 — v1.36.0 PUBLICADA (ROLLING, no forzada)
**Release:** csd-app en `main` + APK 1.36.0 firmado (cert prod `3c5316d8…df5065`) + bucket (`csd-app-1.36.0.apk`+`latest`+`version.json`, `MIN_VERSION`=1.34.0 → **NO forzada**) + historial Y1 (9 cambios). SGC: migración `aa20app-combustible-subtipo-rpc.sql` aplicada (RPC `registrar_combustible_app` +`p_subtipo`). **Rollback:** `update sgc.app_versiones set publicada=true where plataforma='movil' and version='1.35.0'; update sgc.app_versiones set publicada=false where plataforma='movil' and version='1.36.0';` (ninguna forzada, así que basta con la publicada).
...
**Pendiente (device-QA / nativo / grande — tramo enfocado):** AA9 (voz por restricción), AA10 (fotos múltiples de daño), AA13 (foto+voz por falla), AA16 (bridge nativo de micrófono), AA17/18/19 (form de vehículo: horómetro km↔horas en todos los flujos, color/aseguradora select, uso obra/oficina, reordenar fotos+portada), transcripción en detalle de bitácora/incidente. Ver `DEVICE-QA-1.36.0.md`.
