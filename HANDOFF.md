...
**+v1.37.0 (30/07):** AA9 (voz por restricción), AA10 (fotos múltiples de equipo dañado), AA13 (foto+voz por falla del reporte semanal) — todos con su contrato server aplicado.
**Pendiente (device-QA / nativo / grande — tramo enfocado):** regla #5 web (mostrar la voz de restricción / fotos múltiples de daño / foto+voz de falla en el detalle web), AA16 (bridge nativo de micrófono), AA17/18/19 (form de vehículo: horómetro km↔horas en todos los flujos, color/aseguradora select, uso obra/oficina, reordenar fotos+portada), transcripción de audios de sub-ítem + en detalle de bitácora/incidente. Ver `DEVICE-QA-1.36.0.md` + `PROMPT-10-ESTADO-CSD-APP.md`.
