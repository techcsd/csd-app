...
## Ronda 5 app — PROMPT-12 / Act.4 (2026-07-22) — v1.23.0 PUBLICADA (no mínima)
...
- **⚠️ ACCIÓN PENDIENTE — marcar 1.23.0 como MÍNIMA tras QA:** ahora está `publicada=true, minima=false` (actualización **opcional**); 1.22.1 sigue como piso mínimo. **Tras verificar en dispositivo, marcarla `minima=true` en SGC** (Admin → versiones, o `update sgc.app_versiones set minima=true where plataforma='movil' and version='1.23.0'` + quitar la mínima anterior). No se forzó automáticamente porque faltaba la QA en device.
- **PENDIENTE device-QA (necesita teléfono/JWT real):** APK + PWA online **y** en avión — resumen legible en todos los wizards; incidente de equipo eligiendo equipo de la lista + comentario obligatorio fuera de servicio (online y offline drena solo); combustible con Total Energies por default.
