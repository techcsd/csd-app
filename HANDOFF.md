...
## Ronda 28/07 PM — PROMPT-7-CSD-APP (IDs Z, CONTEXTO-ACTUALIZACION-1) — 2026-07-29 — FASES 1–6 · v1.34.0 (commit+push+APK) · PUBLICACIÓN/MÍNIMA + DEVICE-QA PENDIENTES
**Estado:** `npm run build` (app) VERDE (solo warnings NG8102 preexistentes) + build web SGC VERDE. **3 contratos SGC aplicados a la BD compartida** (aditivos/retrocompatibles). **Commit+push a `main`** (app + SGC) y **APK 1.34.0 firmado + versión registrada (Y1)**. **NO publicada / NO mínima** (flags = paso manual del admin cuando pase el device-QA) y **NO `apk:publish`** al bucket todavía. `MIN_VERSION` se dejó en 1.33.0 (no se fuerza nuevo mínimo). Resumen de contratos y follow-ups en `C:\developer\improvements\imp 28072026\PROMPT-7-PENDIENTES-SGC.md`. **Rollback (si se llega a publicar y falla device-QA):** `update sgc.app_versiones set publicada=true, minima=true where plataforma='movil' and version='1.33.0'; update sgc.app_versiones set publicada=false, minima=false where plataforma='movil' and version='1.34.0';`
