...
## Ronda 9 app — PROMPT-20 (2026-07-24) — X4/X9/X8/X10 + X6-app/X3-app — v1.28.0 PUBLICADA (NO forzada)
Source: prompt inline de 6 fases (X4 permisos, X9 ping, X8 Face ID PWA, X10 PIN olvidado, X6-app visitas taller, X3-app accidente con fotos). `npm run build` VERDE en cada corte (solo warnings NG8102 preexistentes). Commit **`9c7570f`** en `main` (pusheado → deploy PWA). APK 1.28.0 firmado (cert prod `3c5316d8…df5065`) + bucket (`csd-app-1.28.0.apk`+`latest`+`version.json`) + historial Y1 (6 cambios) + `apk_url`. **1.28.0 PUBLICADA, NO forzada** (`publicada=true`, única publicada; 1.27.0 despublicada; **`minima` sigue 1.27.0**) → 1.27.0 recibe actualización OPCIONAL a 1.28.0 (seguro pre-device-QA, incl. Face ID que necesita iPhone real). **Rollback:** `update sgc.app_versiones set publicada=true where plataforma='movil' and version='1.27.0'` + `publicada=false where version='1.28.0'`. Para **forzar** 1.28.0 tras QA: `update … set minima=true where version='1.28.0'` + `minima=false where version='1.27.0'`. **PROMPT-19-SGC ya estaba aplicado** — verificado por query: `crear_mantenimiento_app` ya tiene `p_tipo`+`p_incluye_preventivo`+`p_accidente_id`; `registrar_accidente_app` ya tiene `p_fotos jsonb`. Las 6 fases eran adelantables.
...
- **PENDIENTE (device-QA — necesita teléfono/iPhone real; nada de código):**
  ...
  - [x] ~~bump + release + Y1~~ — **hecho** (1.28.0 publicada, NO forzada; commit `9c7570f`). Tras device-QA OK: forzar mínima a 1.28.0 si se quiere obligar la actualización.
