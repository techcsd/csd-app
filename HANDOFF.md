...
- **PUBLICADA a usuarios: 1.17.0** · **MÍNIMA FORZADA: 1.17.0** (gate BLOQUEANTE — todos deben actualizar; trae los fixes de mic/GPS/outbox). `version_publicada(movil)` → publicada 1.17.0 / minima 1.17.0 (code 1017000). 1.16.0 quedó despublicada y `minima=false`. APK firmado (cert prod `3c5316d8…df5065`) subido al bucket (`csd-app-1.17.0.apk` + latest + version.json), `apk_url` OK, historial `app_versiones` (movil) con 12 cambios estructurados. `MIN_VERSION` de `release-apk.mjs` alineado a 1.17.0. Commits en `main`: `d725b9e` (feat) · `66a26f5` (handoff) + este (MIN_VERSION). Push a `main` dispara el deploy PWA (iOS).
