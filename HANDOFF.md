...
## Ronda 9 app — PROMPT-20 (2026-07-24) — X4/X9/X8/X10 + X6-app/X3-app — BUILD VERDE, sin release aún
Source: prompt inline de 6 fases (X4 permisos, X9 ping, X8 Face ID PWA, X10 PIN olvidado, X6-app visitas taller, X3-app accidente con fotos). `npm run build` VERDE en cada corte (solo warnings NG8102 preexistentes). **NADA commiteado ni publicado** (esperando OK + device-QA). **PROMPT-19-SGC ya estaba aplicado** — verificado por query: `crear_mantenimiento_app` ya tiene `p_tipo`+`p_incluye_preventivo`+`p_accidente_id`; `registrar_accidente_app` ya tiene `p_fotos jsonb`. Las 6 fases eran adelantables.
...
- **PENDIENTE (device-QA — necesita teléfono/iPhone real; nada de código):**
  ...
  - [ ] Tras QA OK: bump + release + Y1 (aún NO hecho).
