...
## Ronda QA app (2026-07-20) — v1.17.0 PUBLICADA (P1–P13: permisos, outbox, UX, flota, inventario, auditoría)
...
- **PUBLICADA a usuarios: 1.17.0** (rollout NO bloqueante — banner "nueva versión"). **Mínima forzada: 1.16.0** (SIN cambiar — el admin la tenía en 1.16.0; ojo: el comentario `MIN_VERSION` de `release-apk.mjs` dice 1.15.0 pero la BD manda; la fila `minima=true` es 1.16.0). APK firmado (cert prod `3c5316d8…df5065`) subido al bucket (`csd-app-1.17.0.apk` + latest + version.json), `apk_url` OK, historial `app_versiones` (movil) con 12 cambios estructurados. `version_publicada(movil)` → publicada 1.17.0 / minima 1.16.0. Commit `d725b9e` en `main` (dispara deploy PWA iOS). 1.16.0 quedó despublicada.
- **Si quieres forzar 1.17.0 como mínima** (por el fix de mic/GPS/outbox): en SGC pon `minima=true` en la fila movil 1.17.0 (y `minima=false` en 1.16.0), o dímelo y lo hago (un PATCH). No lo forcé porque "publicar" ≠ "forzar mínima" (regla Y1).
