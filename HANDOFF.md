...
## Ronda 7 app — PROMPT-15 / Act.6 (2026-07-22) — v1.25.0 PUBLICADA (no forzada), APK subido, historial Y1
Source: `C:\developer\improvements\imp 20072026\CONTEXTO-ACTUALIZACION-6.md` (V1–V8) + `PROMPT-15-CSD-APP.md`. `npm run build` VERDE por fase y final (0 errores; 6 warnings NG8102 preexistentes). Commit **`2f12395`** en `main` (push → deploy PWA). APK 1.25.0 firmado (cert prod `3c5316d8…df5065`), subido al bucket `app-releases` (`csd-app-1.25.0.apk` + `latest` + `version.json`), historial registrado (Y1, 8 cambios) + `apk_url`. **1.25.0 PUBLICADA, NO forzada** (`publicada=true, minima=false`; 1.24.0 despublicada; `version_minima` sigue **1.24.0**) → 1.24.0 recibe actualización OPCIONAL a 1.25.0 (seguro pre-QA); los <1.24.0 siguen forzados. **Rollback:** `update app_versiones set publicada=true where version='1.24.0'` + `publicada=false where version='1.25.0'`. **PROMPT-16-SGC NO fue necesario** (V2/V3 con consultas RLS-scoped directas; sin cambios de server).
...
- **Nota (V2 "tap al detalle"):** la app no tiene pantalla de detalle por checklist/echada; las filas muestran fecha+vehículo+veredicto/rendimiento (cumple "ver historial"); rutas creadas llevan a Conduces. Detalle dedicado = follow-up opcional.
- **PENDIENTE device-QA (APK 1.25.0 + PWA; necesita teléfono/JWT real):**
