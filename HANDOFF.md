...
**FASE 5 — Mapas (AG10):** **Leyenda ✅ HECHA** en el Seguimiento del app (`seguimiento.*`): overlay colapsable con los 6 colores de estado AF28 + símbolo de ruta activa (mismos colores que la web). **Google Maps vs Leaflet: NO migrado** — depende de tu decisión tras el research de PROMPT-5 FASE 8 (aún no tomada) → saltado, anotado.
...
2. **Commit/push del código app** (todo el working tree salvo FASE 1 ya commiteada) — espera tu OK.
3. **APK release** (`npm run apk` registra versión Y1) — espera tu OK; necesario para device-QA del background/tracking.
