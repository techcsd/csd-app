...
## 🟡 SESIÓN 15/09/2026 — PROMPT-51 ronda BQ — **BQ7 diagnóstico (Felix→AF18) + barra con tipo/deep-link + BQ6 fecha local + BL9 + molde-esquema v2 (formas) + BQ2 texto + BQ4 verificado + BQ10 deep-link Compa** · build+tokens verde · SIN commit/push (gateado a Xaviel)
...
- **F0 device-QA de 2.21.0** — requiere device/PWA físico (sesión de fondo, sin ADB). **Primero al volver con device.**
- **F3.2 multi-tramo ("+ Agregar lado")** — DIFERIDO: additivo al wizard del parte; v2 ya dibuja las formas bien desde un tramo. Mejor con device-QA.
- **F4 BQ8(c) compositor de moldes** — el padre YA construyó `SGC/src/shared/ui/molde-compositor` (.ts/.html/.scss). Listo para portar, PERO F4 exige **mock BD1 móvil (360px) aprobado por Xaviel** + ergonomía táctil validada en device (handles ≥56px, teclado numérico grande, snap 5cm, deshacer). No se porta a ciegas (gate de aprobación + verify). **Pendiente de OK de Xaviel.**
