...
**Bump de versión (esta sesión):** `1.3.2`→**`1.4.0`** / versionCode 16→**17** en `android/app/build.gradle`, `src/environments/*`, `scripts/release-apk.mjs` (+changelog). El PWA ya está en prod con esta versión. **El APK nativo NO se pudo compilar aquí** (falta JDK 21 / Android Studio / keystore): pendiente que Xaviel corra la secuencia de `scripts/build-apk.md` (`npm run build` → `npx cap sync android` → `gradlew assembleRelease` → `node scripts/release-apk.mjs`). Publicar el APK solo lo hace *disponible*; qué versión ven los usuarios de campo lo decide el admin en SGC → app-versiones (R15).
