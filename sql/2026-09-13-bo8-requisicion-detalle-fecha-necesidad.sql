-- BO8 — expose fecha_necesidad in requisicion_detalle (app "Mis requisiciones" detail).
-- ADDITIVE + backward-compatible: only adds one key to the returned jsonb. The web
-- ignores unknown keys; old app builds never read it. No RLS/grant change (same
-- SECURITY DEFINER function, same authorization gate). PROMPT-44 FASE 6 already added
-- the column solicitudes_material.fecha_necesidad + the write path (crear_solicitud_app);
-- this only surfaces it on read.
--
-- MIRROR (regla madre #5): copy this same file into the SGC repo's sql/ so the shared
-- backend history stays in sync. The web detail view can render it too (parity).
...
begin
  ...
  select jsonb_build_object(
    ...
    -- BO8 — fecha en que se necesita el material (nullable).
